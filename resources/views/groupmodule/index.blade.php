...
@section('content')
    ...
    <p>
        <a href="{{ route('groupmodule.create') }}" class="btn btn-sm btn-primary input" title="Create Group Module">Create</a>
    </p>
    @endif
                ...
                @foreach ($data as $datas)
                    <tr>
                        ...
                        <td class="text-center">
                            <a href="{{ route('groupmodule.edit',['id' => $datas['id']]) }}" class="btn btn-sm btn-primary input" title="Edit Group Module">Ubah</a>
                        </td>
                        @endif
                        @if(\App\Models\User::find(session()->get('sesiuserid'))->GroupUser->Module()->where('route',Route::currentRouteName())->first()->pivot->hapus == 'Y')
                        <td class="text-center">
                            <a href="{{ route('groupmodule.delete',['id' => $datas['id']]) }}" class="btn btn-sm btn-danger input" title="Delete Group Module">Hapus</a>
                        </td>
                        @endif
                    </tr>
                @endforeach
